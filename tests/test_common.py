...
def test_get_buckets():
    ...
    assert len([b for b in buckets.values() if 'extramural_bucket' in b]) == 1, 'should have 1 external bucket'
    ...
    assert buckets['external-dev'] == {'extramural_bucket': True, 'extramural_uploader': 'foo@bar.edu', 'name': 'external-dev', 'credentials': {'aws_access_key_id': 'AAA', 'aws_secret_access_key': 'BBB', 'endpoint_url': 'https://some.external/'}, 'is_data_upload': False}
